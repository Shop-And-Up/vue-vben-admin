<script setup lang="ts">
  ...
  import { badgeIcons } from './config';

  const [registerModal, { openModal }] = useModal();

  // Show badge section
  const { value: isEnableVal } = useField('setting.show_badge');
  ...
  // Show heading title
  const { value: showHeaderVal } = useField('setting.badge_heading_visible');
  ...
  // Title content
  const { value: headerVal } = useField('setting.badge_heading');

  // Title Color
  const { value: headerColorVal } = useField('setting.badge_heading_color');

  // Title Align
  // Shopify is not support align in Text Editor
  const { value: headerAlignVal } = useField('setting.badge_heading_text_align');
  const alignList = ref([
  ...
  const { value: headerSizeVal } = useField('setting.badge_heading_font_size');

  // Badge icon size
  const { value: iconVal } = useField('setting.badge_icon_size');

  // Badge icons
  const { value: formIcons } = useField<Array<any>>('custom_setting.badge_icons');
  ...
  const iconSrc = (url) => {
    return badgeIcons.find((c) => c.icon == url)?.icon;
  };
</script>
...
<template>
  <div class="grid grid-cols-2 gap-4">
    <div class="col-span-2 flex justify-between">
      <div class="text-sm">Show badge section?</div>
      <div>
        <Switch
          size="small"
    ...
    <div class="col-span-2">
      <div class="font-bold mb-2">Badge listing</div>
      <draggable
  ...
  <ManageIcon type="badge" v-model:iconSelected="formIcons" @register="registerModal" />
</template>
